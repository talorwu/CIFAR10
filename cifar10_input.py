...
之前的教程中，tensorflow的模型输入都是使用placeholder来实现的，但是这种情况不适用于大数据量的情况。
因此本篇教程介绍了tensorflow的另一种数据读取机制，从文件中读取数据，这是一种更通用的方法。

大体的实现思路是，让tensorflow单独创建一个queue runner线程，它负责从文件中读取样本数据，并将其装载到一个队列中。
我们只需要开启这个线程，在需要数据时从队列中获取想要的size的数据集就可以了，队列数据的装载由该线程自动实现的。
...
def _generate_image_and_label_batch(image, label, min_queue_examples,
  ...
  #展示训练图片
  #tf.summary.image(tag, tensor, max_images=3, collections=None, name=None)所以只显示3张
